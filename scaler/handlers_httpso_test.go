...
func TestStreamIsActive_HTTPSO(t *testing.T) {
...
func TestIsActive_HTTPSO(t *testing.T) {
	type testCase struct {
		name           string
		expected       bool
		expectedErr    bool
...
func TestGetMetricSpec_HTTPSO(t *testing.T) {
	const ns = "testns"
	type testCase struct {
		name           string
	...
	cases := []testCase{
		{
			...
			checker: func(t *testing.T, res *externalscaler.GetMetricSpecResponse, err error) {
				...
				r.Equal(MetricNameHTTPSO(&types.NamespacedName{Namespace: ns, Name: validHTTPScaledObjectName}), spec.MetricName)
		...
		},
		{
			...
			checker: func(t *testing.T, res *externalscaler.GetMetricSpecResponse, err error) {
				...
				r.Len(res.MetricSpecs, 1)
				spec := res.MetricSpecs[0]
				r.Equal(MetricNameHTTPSO(&types.NamespacedName{Namespace: ns, Name: t.Name()}), spec.MetricName)
...
func TestGetMetrics_HTTPSO(t *testing.T) {
	const (
		ns = "default"
	)
	...
	testCases := []testCase{
		{
			...
			checkFn: func(t *testing.T, res *externalscaler.GetMetricsResponse, err error) {
				...
				r.Equal(MetricNameHTTPSO(&types.NamespacedName{Namespace: ns, Name: validHTTPScaledObjectName}), metricVal.MetricName)
		...
		},
		{
			...
			checkFn: func(t *testing.T, res *externalscaler.GetMetricsResponse, err error) {
				...
				r.Len(res.MetricValues, 1)
				metricVal := res.MetricValues[0]
				r.Equal(MetricNameHTTPSO(&types.NamespacedName{Namespace: ns, Name: t.Name()}), metricVal.MetricName)
				// the value here needs to be the same thing as
				// the sum of the values in the fake queue created
				// in the setup function
