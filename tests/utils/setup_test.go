...
func TestSetupIngress(t *testing.T) {
	...
	_, err = ExecuteCommand(fmt.Sprintf("helm upgrade --install %s ingress-nginx/ingress-nginx --set fullnameOverride=%s --set controller.service.type=ClusterIP --set controller.minReadySeconds=120 --namespace %s --wait",
