...
test.serial('Deploy KEDA and KEDA HTTP Addon', t => {
    ...
    result = sh.exec(`helm install keda kedacore/keda --create-namespace --namespace ${namespace} --set watchNamespace=${namespace}`)
    if (result.code !== 0) {
        t.fail("error deploying KEDA. " + result)
    }
    result = sh.exec(`helm install http-add-on kedacore/keda-add-ons-http --create-namespace --namespace ${namespace}`)
    if (result.code !== 0) {
        t.fail("error deploying KEDA HTTP Addon. " + result)
    }
    t.pass('KEDA and KEDA HTTP Addon deployed successfully')
})
