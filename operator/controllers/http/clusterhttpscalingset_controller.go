...
// ClusterHTTPScalingSetReconciler reconciles a ClusterHTTPScalingSet object
//
//revive:disable-next-line:exported
//goland:noinspection GoNameStartsWithPackageName
type ClusterHTTPScalingSetReconciler struct {
	client.Client
	Scheme        *runtime.Scheme
	KEDANamespace string
}

// +kubebuilder:rbac:groups=http.keda.sh,resources=clusterhttpscalingsets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=http.keda.sh,resources=clusterhttpscalingsets/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=http.keda.sh,resources=clusterhttpscalingsets/finalizers,verbs=update

// Reconcile reconciles a newly created, deleted, or otherwise changed
// HTTPScaledObject
func (r *ClusterHTTPScalingSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	logger := log.FromContext(ctx, "httpscalingset", req.NamespacedName)
	logger.Info("Reconciliation start")

	httpss := &httpv1alpha1.ClusterHTTPScalingSet{}
	...
	// we set the namespace for the new resources via ClusterHTTPScalingSet's namespace
...
func (r *ClusterHTTPScalingSetReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&httpv1alpha1.ClusterHTTPScalingSet{}, builder.WithPredicates(
			predicate.Or(
