...
- **Interceptor**: Add `interceptor.routing_table.last_rebuild_timestamp` metric, reporting when the routing table was last rebuilt, so a stalling refresh loop can be alerted on before readiness takes the pod out of the Service ([#1741](https://github.com/kedacore/http-add-on/issues/1741))
...
- **Interceptor**: `/readyz` now fails when the routing table's refresh loop stops making progress, instead of staying ready forever once the initial sync has passed. The table rebuilds itself every `KEDA_HTTP_ROUTING_TABLE_REFRESH_INTERVAL` (default `1m`), so the check also holds for an interceptor watching zero routes, and readiness fails after `KEDA_HTTP_ROUTING_TABLE_MAX_STALENESS` (default `5m`) without a successful rebuild. Set the staleness to `0` to disable the check ([#1741](https://github.com/kedacore/http-add-on/issues/1741))
...
- **Scaler**: Emit external-push activation events only on inactive-to-active transitions, preventing repeated active status updates from delaying scale-to-zero cooldown ([#1792](https://github.com/kedacore/http-add-on/issues/1792))
