...
- **Interceptor**: Bound the cold-start hold queue with new InterceptorRoute fields `coldStart.maxQueueDepth` and `coldStart.overflow` (`Reject` | `Placeholder`), plus a global `KEDA_HTTP_COLD_START_MAX_QUEUE_DEPTH` env var (default `0`, unlimited). The cap applies per interceptor replica, so effective cluster-wide hold capacity scales with the replica count. Requests arriving while the queue is full get `503` with `Retry-After` or the route's placeholder response, and are counted by the new `interceptor.coldstart.rejected` metric ([#1732](https://github.com/kedacore/http-add-on/issues/1732))
